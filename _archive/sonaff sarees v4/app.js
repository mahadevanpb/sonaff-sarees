...
const EMAILJS_PUBLIC_KEY  = 'YOUR_PUBLIC_KEY';  // e.g. xxxxxxxxxxxxxxxx
...
function handleContact(event) {
  ...
  // Check if EmailJS SDK is loaded and keys are configured
  if (typeof emailjs !== 'undefined' && EMAILJS_SERVICE_ID !== 'YOUR_SERVICE_ID') {
    emailjs.sendForm(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, form, EMAILJS_PUBLIC_KEY)
      ...
      .catch((error) => {
        ...
        if (submitBtn) { submitBtn.textContent = 'Send Message'; submitBtn.disabled = false; }
        showToast("⚠ Failed to send message. Please try WhatsApp instead.");
      });
  } else {
    // Fallback demo mode when keys are not set yet
    console.warn('EmailJS not configured yet. Using demo fallback mode.');
    setTimeout(() => {
      form.reset();
      if (successEl) successEl.classList.remove('hidden');
      if (submitBtn) { submitBtn.textContent = 'Send Message'; submitBtn.disabled = false; }
      showToast("✓ [DEMO] Message sent! Configure EmailJS keys in app.js to send real emails.");
    }, 1200);
  }
}
